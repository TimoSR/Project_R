...
resource "google_project_iam_member" "secret_accessor" {
  ...
  member  = "serviceAccount:459671476145-compute@developer.gserviceaccount.com"
}
...
resource "google_cloud_run_service" "user-management-service" {
  name     = "user-management-service" # Replace with your desired service name.
  provider = google
  location = var.region
...
resource "google_service_account" "sa" {
  account_id   = "usr-mgmt-pubsub-inv"  # Shortened to meet the character limit
  display_name = "User Management Pub/Sub Invoker"
...
resource "google_cloud_run_service_iam_binding" "binding" {
  location = google_cloud_run_service.user-management-service.location
  service  = google_cloud_run_service.user-management-service.name
  role     = "roles/run.invoker"
  members  = [
    "allUsers",
